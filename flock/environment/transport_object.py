...
class TransportObject:
    ...
    def check_if_point_inside(self, point):
        ...
        return (
            point[0] >= self.position[0] - half_width and
            point[0] <= self.position[0] + half_width and
            point[1] >= self.position[1] - half_height and
            point[1] <= self.position[1] + half_height
        )
    ...
    def check_collision_with_agent(self, agent):
        ...
        closest_x = max(self.position[0] - self.width/2, 
                        min(agent.position[0], self.position[0] + self.width/2))
        closest_y = max(self.position[1] - self.height/2, 
    ...
    def check_swept_collision_with_agent(self, agent, dt):
        ...
        current_in_box = (
            agent.position[0] >= self.position[0] - half_width and
            agent.position[0] <= self.position[0] + half_width and
            agent.position[1] >= self.position[1] - half_height and
            agent.position[1] <= self.position[1] + half_height
        )
        
        future_position = agent.position + agent_movement
        future_in_box = (
            future_position[0] >= self.position[0] - half_width and
            future_position[0] <= self.position[0] + half_width and
            future_position[1] >= self.position[1] - half_height and
            future_position[1] <= self.position[1] + half_height
        )
    ...
    def render(self, screen):
        ...
        pygame.draw.line(screen, (255, 255, 255), 
                        ...
                        (center[0] + marker_size, center[1]), 2)
        pygame.draw.line(screen, (255, 255, 255), 
                        (center[0], center[1] - marker_size), 
                        (center[0], center[1] + marker_size), 2)