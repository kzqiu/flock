...
class FlockEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.previous_distance = np.linalg.norm(self.transport_object.position - self.target_pos)
    ...
    def step(self, actions):
        ...
        distance_to_target = np.linalg.norm(self.transport_object.position - self.target_pos)
        self.success = distance_to_target < (self.target_radius + self.transport_object.width/2)
    ...
    def render(self, mode='human'):
        ...
        try:
            ...
            if self.screen is not None:
                ...
                # draw target
                pygame.draw.circle(self.screen, (0, 200, 0), self.target_pos.astype(int), self.target_radius)
        ...
        for agent in self.agents:
            ...
            if nearest_obstacle:
                ...
                if np.linalg.norm(dir_from_obstacle) > 0:
                    dir_from_obstacle = dir_from_obstacle / np.linalg.norm(dir_from_obstacle)
    ...
    def create_random_obstacles(self, count):
        ...
        while len(self.obstacles) < count and attempts < max_attempts:
            ...
            dist_to_source = np.linalg.norm(
                np.array([obstacle.position[0] - source_region['x'],
            ...
            dist_to_target = np.linalg.norm(
                np.array([obstacle.position[0] - target_region['x'],
